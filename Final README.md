...
In this project, we are using a ~190GB dataset of Spotify's complete music archive to predict track popularity using distributed computing on SDSC Expanse. The dataset contains over 256 million tracks with metadata spanning artists, albums, audio features, and more. Using Apache Spark and distributed machine learning, we built and compared models such as Random Forest and XGBoost to identify what features drive a song's popularity score on Spotify.
...
#### Model 2
Our second model ran a PCA and XGBoost regression to predict Spotify track popularity. The PCA analysis showed that the dataset had substantial redundancy with the first principle component addressing about 97% of the variance.  But reducing the dataset to only one principal component limited the predictive capability of the model. The model achieved an RMSE of ~4.87 and had a low R^2 score of 0.07; this low score only accounts for a small portion of the variance in popularity score. Overall while the model was efficient and had good scalability, it had a weak predictive performance. Suggesting that dimensionality reduction removed predictive information, leading to the model to underfit.
...
#### Model 2
Several factors may explain the limited performance of the model. First, the use of PCA may have removed important predictive information while retaining variance, as most of the variance was captured in a very small number of components. Training the model directly on the df_clean data set/original features set could improve performance. After improving the model by removing PCA, the XGBoost model more accurately predicts song popularity, with values closely matching the true scores. High-popularity songs receive high predictions, and overall errors remain small, reflected in a strong R² (~0.81) and low RMSE (~2.16). The scatter plot shows a clear positive trend, though the model slightly underestimates higher values and overestimates very low ones. Despite this minor smoothing effect toward the mean, performance is significantly better than the PCA-based model.
Second, the available features may not sufficiently explain track popularity, suggesting the need for additional features such as artist-level statistics, release timing, or genre-based attributes. Third, the XGBoost model was not extensively tuned, and further hyperparameter optimization could improve predictive accuracy. Finally, the model appears to systematically underestimate high popularity values, indicating that it struggles to model extreme outcomes. Addressing this may require more expressive features or alternative modeling approaches.
